...
from BIKprotect._version import __version__
from BIKprotect.config.chat_config import ChatGPTConfig
from BIKprotect.utils.APIs.chatgpt_api import ChatGPTAPI
...
def main():
    parser = argparse.ArgumentParser(description="BIKprotectTestConnection")
    parser.add_argument(
        "--logDir",
        type=str,
        default="logs",
        help="Log file directory for BIKprotectTestConnection",
    )
    ...
    console.print(
        f"You're testing the connection for BIKprotect v{version}", style="bold green"
    )
    ...
    try:
        ...
        console.print(
            "1. You're connected with OpenAI API. You have GPT-3.5 access. To start BIKprotect, please use <BIKprotect --reasoning_model=gpt-3.5-turbo-16k>",
            style="bold green",
        )
    ...
    try:
        ...
        if conversations is not None:
            console.print(
                "3. You're connected with ChatGPT Plus cookie. \nTo start BIKprotect, please use <BIKprotect --reasoning_model=gpt-4>",
                style="bold green",
            )
        else:
            console.print(
